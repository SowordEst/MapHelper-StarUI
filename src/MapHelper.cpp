...
ActionInfoMap g_actionInfoTable = {
	#ifdef StarUI
		#include "StarUI\StarUI_ActionInfoTable.h"
	#endif // StarUI
...
static void __fastcall insertCreateUI(Action* action,uint32_t edx, int flag)
{
	...
	{
#ifdef StarUI
#include "StarUI\StarUI_insertCreateUI.h"
#endif // StarUI
...
int Helper::onSelectConvartMode()
{
	...
	if (result == -1)
	{
		...
		int ret = MessageBoxA(0, "是否用新的保存模式保存?", "地图保存加速", MB_SYSTEMMODAL | MB_YESNO);
...
void Helper::enableConsole()
{
	...
	if (v_hwnd_console)
	{
		...
		::SetWindowTextA(v_hwnd_console, "ydwe保存加速插件 2.2r + StarUI");

		
		std::string text = R"(
<while>
用来加速ydwe保存地图的插件，对地形装饰物，触发编辑器极速优化
参与开发者 ：<yellow>w4454962</yellow>、 神话、 actboy168、月升朝霞、白喵、裂魂、星星
version 2.2r+StarUI update:
<grey>
